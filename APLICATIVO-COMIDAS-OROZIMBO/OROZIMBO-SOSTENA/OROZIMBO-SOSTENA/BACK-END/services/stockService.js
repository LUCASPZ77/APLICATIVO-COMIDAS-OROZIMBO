...
export async function processBaixa({ itens, usuarioCpf, usuarioNome, prato, periodo }) {
  let user = usuarioCpf ? getUserByCpf(usuarioCpf) : null;
  if (!user && usuarioNome) {
    user = getUserByName(usuarioNome);
  }
  ...
  for (const item of itens) {
    const estoqueAtual = getStockById(item.id);
    ...
    updateStock({ id: item.id, quantidade: novaQuantidade, qtd_unidades: novasUnidades, updated_at: timestamp });
...
export async function addManual({ item, lote, quantidade, qtd_unidades, validade }) {
  ...
  addOrUpdateStock({ item, quantidade: Number(quantidade), qtd_unidades: Number(qtd_unidades || 0), lote: lote || 'S/L', validade: validade || '-', created_at, updated_at });
  const estoque = getStockByItemLote(item, lote || 'S/L');
  let usuario = getAnyUser();
  if (!usuario) {
    // cria um usuário sistema mínimo para referenciar nos logs
    createUser({ nome: 'Sistema', sobrenome: '', email: 'sistema@local', cpf: '00000000000', senha_hash: '', cargo: 'Sistema', created_at: now.toISOString(), updated_at: now.toISOString() });
    usuario = getAnyUser();
  }

  const usuario_id = usuario ? usuario.id : null;
  createLog({ item_id: estoque.id, usuario_id, quantidade: Number(quantidade), unidades: Number(qtd_unidades || 0), data, hora, tipo: 'ENTRADA', lote: lote || 'S/L', prato: null, periodo: null, timestamp });
  return estoque;
}
...
export async function importExcel({ dados }) {
  ...
  for (const i of dados) {
    ...
    upsertStock({ item: nome, quantidade: qtdKg, qtd_unidades: qtdUn, lote, validade: i.validade || '-', created_at, updated_at });
    const estoque = getStockByItemLote(nome, lote);
    let usuario = getAnyUser();
    if (!usuario) {
      createUser({ nome: 'Sistema', sobrenome: '', email: 'sistema@local', cpf: '00000000000', senha_hash: '', cargo: 'Sistema', created_at: now.toISOString(), updated_at: now.toISOString() });
      usuario = getAnyUser();
    ...
    const usuario_id = usuario ? usuario.id : null;
    createLog({ item_id: estoque.id, usuario_id, quantidade: qtdKg, unidades: qtdUn, data, hora, tipo: 'ENTRADA', lote, prato: null, periodo: null, timestamp });
  }
}

export function editStock({ id, quantidade, qtd_unidades, updated_at }) {
